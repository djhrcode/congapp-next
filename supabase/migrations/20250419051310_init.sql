DO $$ BEGIN
 CREATE TYPE "public"."meeting_type" AS ENUM('midweek', 'weekend', 'memorial');
EXCEPTION
...
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS "congregation_attendance_reports" (
	"id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY (sequence name "congregation_attendance_reports_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 10000 CACHE 1),
	"virtual_attendance" smallint,
	"inperson_attendance" smallint,
	"meeting_type" "meeting_type",
	"congregation_id" varchar,
	"meeting_date" date
);
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS "congregation_monthly_reports" (
	"id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY (sequence name "congregation_monthly_reports_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 10000 CACHE 1),
	"active_publishers" smallint,
	"midweek_total_attendance" smallint,
	"midweek_total_meetings" smallint,
	"midweek_avg_attendance" smallint,
	"weekend_total_attendance" smallint,
	"weekend_total_meetings" smallint,
	"weekend_avg_attendance" smallint,
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "congregation_attendance_reports" ADD CONSTRAINT "congregation_attendance_reports_congregation_id_congregations_id_fk" FOREIGN KEY ("congregation_id") REFERENCES "public"."congregations"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "congregation_monthly_reports" ADD CONSTRAINT "congregation_monthly_reports_congregation_id_congregations_id_fk" FOREIGN KEY ("congregation_id") REFERENCES "public"."congregations"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
END $$;
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "unique_attendance_report_by_cong" ON "congregation_attendance_reports" USING btree ("meeting_date","meeting_type","congregation_id");--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "unique_cong_report_by_period" ON "congregation_monthly_reports" USING btree ("year","month","congregation_id");--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "unique_person_by_period" ON "reports" USING btree ("period","person_id");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "index_by_service_year" ON "reports" USING btree ("service_year");--> statement-breakpoint
