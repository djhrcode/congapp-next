...
interface ErrorParser {
    (error: { key: any; value: unknown; type: string }): string;
}

interface Refiner<Type> {
...
const dataTypes: Record<string, FieldOptions<any, false, false>> = {
    string: {
        defaulted: "",
    },
    number: {
...
>(
    ...
    if (!defaulted) {
        ...
        if (type !== "object" && type !== "array")
            result = lib.defaulted(lib.optional(result), undefined);
    }
...
function makeDefaultSchema<Schema>(
    ...
    if (isArrayStruct(struct)) {
        return lib.defaulted(struct, []) as lib.Struct<Schema>;
    }
...
function form<Schema extends ObjectSchema>(
    ...
    Object.defineProperty(schemaStruct, $$getDefaultState, {
        value(): lib.Infer<lib.Struct<ObjectType<Schema>, Schema>> {
            console.log("getDefaultState", schemaStruct);
...
const formSchema = form({
    name: field(lib.string()),
    roles: field(lib.array(lib.string())),
...
function rules<Type>(
    ...
    return (
        refiners.reduce((carry, refiner) => {
            if (carry === null) return refiner(struct);

            return refiner(carry);
        }, null as Nullable<lib.Struct<Type>>) ?? struct
    );
}
...
export function parseStructError(error: lib.StructError): lib.StructError {
    ...
    error.failures = function () {
        return failures().map(parseFailure);
    };
